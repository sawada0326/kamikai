<?php
    ...
    $title = isset($_POST['title']) ? $_POST['title'] : $articleData['title'];
    $category = isset($_POST['category']) ? $_POST['category'] : $articleData['category'];
    $broadcaster = isset($_POST['broadcaster']) ? $_POST['broadcaster'] : $articleData['broadcaster'];
    $caption = isset($_POST['caption']) ? $_POST['caption'] : $articleData['caption'];
    $on_air_date = isset($_POST['on_air_date']) ? $_POST['on_air_date'] : $articleData['on_air_date'];
    $start_time = isset($_POST['start_time']) ? $_POST['start_time'] : $articleData['start_time'];
    $finish_time = isset($_POST['finish_time']) ? $_POST['finish_time'] : $articleData['finish_time'];
    $casts = isset($_POST['casts']) ? $_POST['casts'] : explode(',', $articleData['casts']);
    $tags = isset($_POST['tags']) ? $_POST['tags'] : explode(',', $articleData['tags']);
    $rating = isset($_POST['rating']) ? (int)$_POST['rating'] : (int)$articleData['rating'];
    $reason = isset($_POST['reason']) ? $_POST['reason'] : $articleData['reason'];
    $publish_status = isset($_POST['publish_status']) ? (int)$_POST['publish_status'] : (int)$articleData['publish_status'];
?>
<!DOCTYPE html>
<html lang="ja">
<head>
...
<body>
    ...
    <div class="container">
        ...
        <form method="POST" enctype="multipart/form-data">
            <input type="hidden" name="id" value="<?php echo h($articleData['id']); ?>">
            <input type="hidden" name="over_write" value="1">
            <div class="column_wrapper">
                ...
                <div id="cast_table" class="column">
                    <p>出演者</p>
                    <div>
                        <span><input type="text" name="casts[]" value="<?php if(!empty($casts[0]))echo h($casts[0]); ?>"></span>
                        <span><input type="text" name="casts[]" value="<?php if(!empty($casts[1]))echo h($casts[1]); ?>"></span>
                        <span><input type="text" name="casts[]" value="<?php if(!empty($casts[2]))echo h($casts[2]); ?>"></span>
                        <span><input type="text" name="casts[]" value="<?php if(!empty($casts[3]))echo h($casts[3]); ?>"></span>
                    </div>
                </div>
                ...
                <div class="column">
                    ...
                    <small>※同じ番組名の記事は画像がすべて更新されます</small>
                </div>
